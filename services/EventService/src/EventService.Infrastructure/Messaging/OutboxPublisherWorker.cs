...
public sealed class OutboxPublisherWorker : BackgroundService
{
    ...
    private static Task PublishAsync(
        ...
        {
            nameof(EventCreated) => publisher.PublishAsync(
                Deserialize<EventCreated>(message), cancellationToken),
            nameof(EventUpdated) => publisher.PublishAsync(
                Deserialize<EventUpdated>(message), cancellationToken),
            nameof(EventCancelled) => publisher.PublishAsync(
                Deserialize<EventCancelled>(message), cancellationToken),
            _ => throw new InvalidOperationException(
                $"Outbox message type '{message.Type}' is not supported by EventService.")
        };
