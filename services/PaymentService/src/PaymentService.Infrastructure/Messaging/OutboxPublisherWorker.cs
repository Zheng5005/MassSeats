...
public sealed class OutboxPublisherWorker : BackgroundService
{
    ...
    private static Task PublishAsync(
        ...
        {
            nameof(PaymentSucceeded) => publisher.PublishAsync(
                Deserialize<PaymentSucceeded>(message), cancellationToken),
            nameof(PaymentFailed) => publisher.PublishAsync(
                Deserialize<PaymentFailed>(message), cancellationToken),
            _ => throw new InvalidOperationException(
                $"Outbox message type '{message.Type}' is not supported by PaymentService.")
        };
